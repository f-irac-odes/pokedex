...
// Remove the problematic import - we'll handle this via API instead
// Note: getCurrentUser is only available server-side, so we use API for client-side
...
export async function getCurrentUserInfo() {
    try {
        const result = await apiRequest<{ success: boolean; data: { user: User | null; isAuthenticated: boolean } }>('/api/auth/current');
